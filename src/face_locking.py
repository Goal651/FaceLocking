...
class FaceLockingSystem:
    ...
    def __init__(
        ...
        print(
            f"Face detection: min_size={self.min_face_size}, max_size={self.max_face_size}"
        )
    ...
    def _draw_unlocked_face(
        ...
        if smile_detected:
            ...
            cv2.circle(
                vis, (smile_x - 3, smile_y - 2), 1, (0, 255, 255), -1
            )  # left eye
            cv2.circle(
                vis, (smile_x + 3, smile_y - 2), 1, (0, 255, 255), -1
            )  # right eye
    ...
    def _search_for_target(
        ...
        for face in faces:
            ...
            if smile_detected:
                ...
                cv2.circle(
                    vis, (smile_x - 2, smile_y - 1), 1, (0, 255, 255), -1
                )  # left eye
                cv2.circle(
                    vis, (smile_x + 2, smile_y - 1), 1, (0, 255, 255), -1
                )  # right eye
            ...
            if (
                is_target and match_result.similarity > 0.7
            ):  # High confidence threshold for locking
    ...
    def _draw_locked_face(self, vis: np.ndarray, face: FaceDet):
        ...
        if smile_detected:
            ...
            cv2.circle(
                vis, (smile_x - 4, smile_y - 3), 2, (0, 255, 255), -1
            )  # left eye
            cv2.circle(
                vis, (smile_x + 4, smile_y - 3), 2, (0, 255, 255), -1
            )  # right eye
        ...
        if self.show_landmarks:
            ...
            if len(face.kps) >= 5:
                ...
                cv2.circle(
                    vis, tuple(left_mouth), 3, (0, 255, 255), -1
                )  # left mouth corner
                cv2.circle(
                    vis, tuple(right_mouth), 3, (0, 255, 255), -1
                )  # right mouth corner
                cv2.line(
                    vis, tuple(left_mouth), tuple(right_mouth), (0, 255, 255), 1
                )  # mouth line
    ...
    def _draw_clean_ui(self, vis: np.ndarray):
        ...
        self._draw_clean_text(
            vis,
            f"Face Locking: {status} | Target: {self.target_identity} {mirror_indicator}",
            (10, 25),
            status_color,
            0.7,
            2,
        ...
        if self.show_detailed_info:
            ...
            self._draw_clean_text(
                vis, "D:Show Details | Q:Quit", (10, h - 10), (200, 200, 200), 0.4, 1
            )
    ...
    def adjust_face_detection_sensitivity(self, increase: bool):
        ...
        if increase:
            ...
            self.min_face_size = (
                max(60, self.min_face_size[0] - 10),
                max(60, self.min_face_size[1] - 10),
            )
            self.face_aspect_ratio_range = (
                max(0.5, self.face_aspect_ratio_range[0] - 0.1),
                min(2.0, self.face_aspect_ratio_range[1] + 0.1),
            )
        else:
            # Less sensitive - larger minimum size, narrower aspect ratio range
            self.min_face_size = (
                min(120, self.min_face_size[0] + 10),
                min(120, self.min_face_size[1] + 10),
            )
            self.face_aspect_ratio_range = (
                min(0.9, self.face_aspect_ratio_range[0] + 0.1),
...
def main():
    ...
    try:
        system = FaceLockingSystem(
            target_identity="Wilson", window_scale=1.0, mirror_mode=True
        )
    ...
    try:
        ...
        print(f"  - Target identity: {system.target_identity}")
        print(
            f"  - Final smile threshold: {system.action_detector.smile_ratio_threshold:.1f}"
        )
